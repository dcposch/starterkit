...
library LibQuery {
  function query(QueryFragment[] memory fragments) internal view returns (uint256[] memory) {
    require(
      fragments[0].queryType == QueryType.Has || fragments[0].queryType == QueryType.HasValue,
      'Has/HasValue required'
    );

    uint256[] memory firstFragmentEntities = fragments[0].queryType == QueryType.HasValue
      ? fragments[0].component.getEntitiesWithValue(fragments[0].value)
      : fragments[0].component.getEntities();
    ...
    for (uint256 i = 0; i < firstFragmentEntities.length; i++) {
      ...
      for (uint256 j = 1; j < fragments.length; j++) {
        ...
        // For HasVaue fragments, include entity if it has the requested value
